...
func parse(logs *log.Logger, fset *token.FileSet, path string) (svc map[string]ServiceDecl, err error) {
	f, err := os.Open(path)
	if err != nil {
		...
		for _, spec := range decl.Specs {
			if tspec, ok := spec.(*ast.TypeSpec); ok {
				if itype, ok := tspec.Type.(*ast.InterfaceType); ok {
					svc[tspec.Name.Name], err = parseServiceInterface(logs, itype)
					if err != nil {
...
func write(logs *log.Logger, path string, svc map[string]ServiceDecl) error {
